...
function gb_get_paging($write_pages, $cur_page, $total_page, $url, $add="")
{
    $str = "";
    if ($cur_page > 1) {
...
// 세션변수 생성
function gb_set_session($session_name, $value)
{
...
// 세션변수값 얻음
function gb_get_session($session_name)
{
...
// 쿠키변수 생성
function gb_set_cookie($cookie_name, $value, $expire)
{
    global $g4;
...
// 쿠키변수값 얻음
function gb_get_cookie($cookie_name)
{
...
function gb_url_auto_link($str)
{
    ...
    $str = preg_replace("/([^(http:\/\/)]|\(|^)(www\.[^[:space:]]+)/i", "\\1<A HREF=\"http://\\2\" TARGET='$gb_config[cf_link_target]'>\\2</A>", $str);
    //$str = preg_replace("/([^(HREF=\"?'?)|(SRC=\"?'?)]|\(|^)((http|https|ftp|telnet|news|mms):\/\/[a-zA-Z0-9\.-]+\.[\xA1-\xFEa-zA-Z0-9\.:&#=_\?\/~\+%@;\-\|\,]+)/i", "\\1<A HREF=\"\\2\" TARGET='$gb_config[cf_link_target]'>\\2</A>", $str);
    // 100825 : () 추가
    // 120315 : CHARSET 에 따라 링크시 글자 잘림 현상이 있어 수정
    if (strtoupper($g4['charset']) == 'UTF-8') {
        $str = preg_replace("/([^(HREF=\"?'?)|(SRC=\"?'?)]|\(|^)((http|https|ftp|telnet|news|mms):\/\/[a-zA-Z0-9\.-]+\.[가-힣\xA1-\xFEa-zA-Z0-9\.:&#=_\?\/~\+%@;\-\|\,\(\)]+)/i", "\\1<A HREF=\"\\2\" TARGET='$gb_config[cf_link_target]'>\\2</A>", $str);
    } else {
        $str = preg_replace("/([^(HREF=\"?'?)|(SRC=\"?'?)]|\(|^)((http|https|ftp|telnet|news|mms):\/\/[a-zA-Z0-9\.-]+\.[\xA1-\xFEa-zA-Z0-9\.:&#=_\?\/~\+%@;\-\|\,\(\)]+)/i", "\\1<A HREF=\"\\2\" TARGET='$gb_config[cf_link_target]'>\\2</A>", $str);
    }
    // 이메일 정규표현식 수정 061004
...
// 파일의 용량을 구한다.
//function gb_get_filesize($file)
function gb_get_filesize($size)
{
...
function gb_get_file($bo_table, $wr_id)
{
    global $g4, $qstr;
...
function gb_get_list($write_row, $board, $skin_path, $subject_len=40)
{
    global $g4, $gb_config;
    global $qstr, $page;
    ...
    $tmp_name = get_text(cut_str($list['wr_name'], $gb_config['cf_cut_name'])); // 설정된 자리수 만큼만 이름 출력
...
// get_list 의 alias
function gb_get_view($write_row, $board, $skin_path, $subject_len=125)
{
...
function gb_search_font($stx, $str)
{
    global $gb_config;
    ...
    $replace = "<span style='background-color:$gb_config[cf_search_bgcolor]; color:$gb_config[cf_search_color];'>\\1</span>";
...
// 제목을 변환
function gb_conv_subject($subject, $len, $suffix="")
{
    return cut_str(get_text($subject), $len, $suffix);
}

// OBJECT 태그의 XSS 막기
function gb_bad120422($matches)
{
    $tag  = $matches[1];
    $code = $matches[2];
...
// 내용을 변환
function gb_conv_content($content, $html)
{
    global $gb_config, $board;
...
// 검색 구문을 얻는다.
function gb_get_sql_search($search_ca_name, $search_field, $search_text, $search_operator='and')
{
    global $g4;

    $str = "";
/* 멀티카테고리를 위한 원본 주석 또는 삭제
    if ($search_ca_name)
...
*/
    ...
    //멀티카테고리를 위한 추가
    if ($search_ca_name) {
        if ($GLOBALS['board']['bo_use_category']) {
            $selects = explode(" ", $GLOBALS['board']['bo_category_list']);
            if ($selects[0] == "sql2mc") {
...
// 게시판 테이블에서 하나의 행을 읽음
function gb_get_write($write_table, $wr_id)
{
...
// 게시판의 다음글 번호를 얻는다.
function gb_get_next_num($table)
{
    // 가장 작은 번호를 얻어
...
// 회원 정보를 얻는다.
function gb_get_member($mb_id, $fields='*')
{
    global $g4;
...
function gb_subject_sort_link($col, $query_string='', $flag='asc')
{
...
// 관리자 정보를 얻음
function gb_get_admin($admin='super')
{
    global $gb_config, $group, $board;
    global $g4;
    ...
    if (($is && !$mb[mb_id]) || $admin == 'super') {
        $mb = sql_fetch("select * from $g4[member_table] where mb_id in ('$gb_config[cf_admin]') limit 1 ");
    }
...
// 관리자인가?
function gb_is_admin($mb_id)
{
    global $gb_config, $group, $board;

    if (!$mb_id) return;

    if($gb_config['cf_admin'] == $mb_id) return 'super';
    if($group['gr_admin'] == $mb_id) return 'group';
    if($board['bo_admin'] == $mb_id) return 'board';
    
    $mb = get_member($mb_id);
    if($mb['mb_level'] > 9) return 'super';
...
function gb_get_category_option($bo_table='')
{
    global $g4, $board;
...
function gb_get_group_select($name, $selected='', $event='')
{
...
function gb_get_yn_select($name, $selected='1', $event='')
{
    $str = "<select name='$name' $event>";
    if ($selected) {
...
// 포인트 부여
function gb_insert_point($mb_id, $point, $content='', $rel_table='', $rel_id='', $rel_action='')
{
    global $gb_config;
    global $g4;
    global $is_admin;
    ...
    if (!$gb_config[cf_use_point]) { return 0; }
...
// 포인트 삭제
function gb_delete_point($mb_id, $rel_table, $rel_id, $rel_action)
{
    global $g4;
...
// 회원 레이어
function gb_get_sideview($mb_id, $name="", $email="", $homepage="")
{
    global $gb_config;
    global $g4;
    ...
    if ($mb_id) {
        ...
        if ($gb_config['cf_use_member_icon']) {
            ...
            if (file_exists($icon_file)) {
                ...
                //$width = $size[0];
                //$height = $size[1];
                $width = $gb_config['cf_member_icon_width'];
                $height = $gb_config['cf_member_icon_height'];
                $tmp_name = "<img src='$icon_file' width='$width' height='$height' align='absmiddle' border='0'>";

                if ($gb_config['cf_use_member_icon'] == 2) // 회원아이콘+이름
...
function gb_view_file_link($file, $width, $height, $content="")
{
    global $gb_config, $board;
    global $g4;
    static $ids;
    ...
    if (preg_match("/\.($gb_config[cf_image_extension])$/i", $file))
    ...
    else if (preg_match("/\.($gb_config[cf_flash_extension])$/i", $file))
    ...
    */
    ...
    //---------------------------------------------------------------------------------------------
    /*
    else if (preg_match("/\.($gb_config[cf_movie_extension])$/i", $file))
        //return "<embed src='$g4[path]/data/file/$board[bo_table]/$file' $attr></embed>";
...
// {img:0} ... {img:n} 과 같은 형식
function gb_view_image($view, $number, $attribute)
{
...
function gb_view_link($view, $number, $attribute)
{
    global $gb_config;
    ...
    if ($view[link][$number][link])
    {
        if (!preg_match("/target/i", $attribute))
            $attribute .= " target='$gb_config[cf_link_target]'";
...
function gb_cut_str($str, $len, $suffix="…")
{
    global $g4;
...
function gb_time_select($time, $name="")
{
...
function gb_check_demo()
{
    global $g4, $member;

    if($member['mb_1'] == "demo" || file_exists("$g4[path]/DEMO"))
        alert("데모 화면에서는 하실(보실) 수 없는 작업입니다.");
}


// 문자열이 한글, 영문, 숫자, 특수문자로 구성되어 있는지 검사
function gb_check_string($str, $options)
{
    global $g4;
...
function gb_cut_hangul_last($hangul)
{
    global $g4;
...
// 악성태그 변환
function gb_bad_tag_convert($code)
{
    global $view;
...
function gb_login_check($mb, $mb_id, $mb_password) {
	global $gb_config, $g4;

	if (!trim($mb_id) || !trim($mb_password))
		gb_alert("회원아이디나 패스워드가 공백이면 안됩니다.");

	if ($check_time = gb_get_session("ss_login_check_time")) {
		if ($check_time > $g4['server_time'] - 15) {
			gb_alert("로그인 실패시에는 15초 이후에 다시 로그인 하시기 바랍니다.");
		}
	}
	gb_set_session("ss_login_check_time", $g4['server_time']);

	if (!$mb['mb_id'] || (gb_sql_password($mb_password) !== $mb['mb_password'])) {
		gb_alert("가입된 회원이 아니거나 패스워드가 틀립니다.\\n\\n패스워드는 대소문자를 구분합니다.");
	}

	if ($mb['mb_intercept_date'] && $mb['mb_intercept_date'] <= date("Ymd", $g4['server_time'])) {
		$date = preg_replace("/([0-9]{4})([0-9]{2})([0-9]{2})/", "\\1년 \\2월 \\3일", $mb[mb_intercept_date]); 
		gb_alert("회원님의 아이디는 접근이 금지되어 있습니다.\\n\\n처리일 : $date");
	}

	if ($mb['mb_leave_date'] && $mb['mb_leave_date'] <= date("Ymd", $g4['server_time'])) {
		$date = preg_replace("/([0-9]{4})([0-9]{2})([0-9]{2})/", "\\1년 \\2월 \\3일", $mb['mb_leave_date']); 
		gb_alert("탈퇴한 아이디이므로 접근하실 수 없습니다.\\n\\n탈퇴일 : $date");
	}

	if ($gb_config['cf_use_email_certify'] && !preg_match("/[1-9]/", $mb['mb_email_certify']))
		gb_alert("메일인증을 받으셔야 로그인 하실 수 있습니다.\\n\\n회원님의 메일주소는 {$mb['mb_email']} 입니다.");

	gb_set_session('ss_mb_id', $mb['mb_id']);
	gb_set_session('ss_mb_key', md5($mb['mb_datetime'] . $_SERVER['REMOTE_ADDR'] . $_SERVER['HTTP_USER_AGENT']));
}
